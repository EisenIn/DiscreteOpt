...
% \item \emph{Add this if there is room} Suppose we are given an oracle that tells us whether a
...
%   satisfied by every feasible $x^* ∈ ℝ^n$. The system $A_1 x ≤b_1$ is
%   called the \emph{implicit equalities} of $Ax ≤b$.
%   \begin{enumerate}[i)] 
%   \item If $Ax ≤b$ is feasible, then there exists a feasible solution $x^*$ such that $A_1x^* < b_2$ holds.
