...
\item Let $D = (V ,A)$ be a \textit{directed} graph. Its node-arc incidence matrix is the matrix $M ∈
\{−1,0,1\}^{V ×A}$ defined via
$$M_{w ,(u,v )}= \begin{cases} -1 & \text{ if } w = u \\ 1 & \text{ if } w = v \\ 0 & \text{ otherwise }\end{cases}$$ 
\smallskip

Show that the node-arc incidence matrix of any digraph is totally unimodular.
