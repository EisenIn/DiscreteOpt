...
\duedate{ }
\problemset{Problem Set~2 (Solutions)}
...
\item \label{absolute-value-modeling} 
...
\begin{align*}
    \min &\quad 2x_1 + 3y_1 \\
    \text{subject to} &\quad y_1 \geq x_2 - 10 \\
    &\quad y_1 \geq -(x_2 - 10) \\
    &\quad y_2 \geq x_1 + 2 \\
    &\quad y_2 \geq -(x_1 + 2) \\
    &\quad y_3 \geq x_2 \\
...
\item \label{meat-packing-modeling} 
A meat packing plant produces 480 hams, 400 pork bellies, and 230 picnic hams every day; each of these products can be sold fresh or smoked. 
...
& Fresh & \shortstack{Smoked on \\ regular time} & \shortstack{Smoked on \\ overtime} \\
\cline{2-4}
Hams & $x_{11}$ & $x_{12}$ & $x_{13}$ \\
Pork bellies & $x_{21}$ & $x_{22}$ & $x_{23}$ \\
Picnic hams & $x_{31}$ & $x_{32}$ & $x_{33}$ \\
\hline
\end{tabular}  
\end{center}
Then the problem can be reformulated as
\begin{align*}
    \max &\quad 8x_{11} + 14x_{12} + 11x_{13} + 4x_{21} + 12x_{22} + 7x_{23} + 4x_{31} + 13x_{32} + 9x_{33} \\
    \text{subject to} &\quad x_{11} + x_{12} + x_{13} = 480 \\  
    &\quad x_{21} + x_{22} + x_{23} = 400 \\
    &\quad x_{31} + x_{32} + x_{33} = 230 \\
    &\quad x_{12} + x_{22} + x_{32} \leq 420 \\
    &\quad x_{13} + x_{23} + x_{33} \leq 250 \\
    &\quad x_{ij} \geq 0 \quad \forall \; i, j = 1,2,3.
\end{align*}       
\end{solution}

\item \label{unit-ball-extreme-points}
...
\item \label{intersection-of-convex-sets}
Let $\{C_i\}_{i\in I}$ be a family of convex subsets of $\setR^n$.
...
\item \label{lp-feasible-region-convex}
Show that the set of feasible solutions  of a linear program  is
convex. 
