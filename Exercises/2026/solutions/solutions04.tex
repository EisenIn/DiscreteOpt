...
\duedate{ }
\problemset{Problem Set~4 (Solutions)}
...
The vector 
$$ v= x_1 + 3 x_2 + 2 x_3 + x_4 + 3 x_5 =  \begin{pmatrix}  15\\ 14 \\ 25 \end{pmatrix} $$
...
\item For any vector $x \in \setR^n$, define $\tilde{x} := \begin{pmatrix} x \\ 1 \end{pmatrix} \in \mathbb{R}^{n+1}$. 
Given a set of vectors $X \subseteq \setR^n$ and a vector $y \in \mathbb{R}^{n}$, show that $y \in \mathrm{conv}(X)$ if and only if $\tilde{y} \in \mathrm{cone}(\{\tilde{x}: x\in X\})$.

\begin{solution}
\begin{align*}
y \in \mathrm{conv}(X) 
&\iff \exists \lambda_1, \dots, \lambda_k \geq 0, x_1, \dots, x_k \in X, \text{such that}\, \sum_{i=1}^k \lambda_i = 1, y = \sum_{i=1}^k \lambda_i x_i\\
&\iff \exists \lambda_1, \dots, \lambda_k \geq 0, x_1, \dots, x_k \in X, \text{such that}\, \tilde{y} = \begin{pmatrix} y \\ 1 \end{pmatrix} = \sum_{i=1}^k \lambda_i \begin{pmatrix} x_i \\ 1 \end{pmatrix} = \sum_{i=1}^k \lambda_i \tilde{x}_i \\
&\iff \tilde{y} \in \mathrm{cone}(\{\tilde{x}: x\in X\}).
\end{align*}
\end{solution}
