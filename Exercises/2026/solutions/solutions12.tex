...
\begin{solution}
\begin{enumerate}[i)]
\item We prove the statement by induction on $k$.

For $k=1$, the claim is exactly the definition of the adjacency matrix: $a_{uv}=1$ if and only if there is a directed walk of length $1$ from $u$ to $v$, and otherwise $a_{uv}=0$.

Assume now that the claim holds for some $k\geq 1$. Then
\[
...
\item Compute $A^k$ by repeated squaring. Writing $k$ in binary, one needs $O(\log k)$ matrix multiplications: successively compute
\[
A, A^2, A^4, A^8,\ldots
\]
and multiply the powers corresponding to the $1$-bits in the binary expansion of $k$.

Using a matrix multiplication algorithm with arithmetic complexity $O(n^{2.805})$ for multiplying two $n\times n$ matrices, the total number of arithmetic operations is
\[
O(\log k)\cdot O(n^{2.805})=O(\log k\cdot n^{2.805}).
\]
\end{enumerate}
\end{solution}
...
\begin{solution}
Let $t$ denote the number of triangles in $G$. Since $G$ is undirected and simple,
\[
\operatorname{tr}(A^3)=6t,
\]
because every triangle contributes exactly six closed directed walks of length $3$, corresponding to the two orientations and the three choices of starting vertex. Conversely, every closed walk of length $3$ in a simple undirected graph comes from a triangle.

Choose uniformly at random a prime $p>3$ among the first
\[
m:=4\lceil \log_2 n\rceil
\]
primes larger than $3$. Compute $A^3$ over the field $\mathbb{F}_p$, and assert \emph{Yes} if
\[
\operatorname{tr}(A^3)\not\equiv 0 \pmod p.
\]
Otherwise assert \emph{No}.

We verify the required properties.

If $G$ has no triangle, then $t=0$, so $\operatorname{tr}(A^3)=0$ over the integers. Therefore its reduction modulo every prime $p$ is also $0$, and the algorithm always asserts \emph{No}.

If the algorithm asserts \emph{Yes}, then $\operatorname{tr}(A^3)\not\equiv 0\pmod p$. In particular, $\operatorname{tr}(A^3)\neq 0$ over the integers. Hence $t>0$, so $G$ contains a triangle.

Now assume that $G$ contains at least one triangle. Then $1\leq t\leq \binom n3<n^3$. Since $p>3$, the condition
\[
\operatorname{tr}(A^3)=6t\equiv 0\pmod p
\]
is equivalent to $p\mid t$. The integer $t$ has at most
\[
\log_5(n^3)=3\log_5 n < 2\log_2 n
\]
distinct prime divisors larger than $3$, because each such prime is at least $5$. Among the $m=4\lceil\log_2 n\rceil$ possible primes, fewer than half can divide $t$. Thus the probability that the chosen prime divides $t$, and hence the probability that the algorithm incorrectly asserts \emph{No}, is at most $1/2$.

Finally, the largest of the first $m=O(\log n)$ primes is $O(m\log m)=O(\log n\log\log n)$, so the modulus $p$ has bit length $O(\log\log n)$. Computing $A^3$ modulo $p$ requires two matrix multiplications over $\mathbb{F}_p$, hence
\[
O(n^{2.805})
\]
arithmetic operations on integers of size $O(\log\log n)$.
\end{solution}
...
where $B'$ is a smaller square submatrix of $A_G$. By induction hypothesis,
