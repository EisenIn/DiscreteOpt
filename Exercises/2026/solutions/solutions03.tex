...
\item \label{convex-hull}
Given a set of vectors $X \subseteq \setR^n$, show that the convex hull of $X$ is convex. 

\begin{solution}
Let $x, y \in \conv(X)$. Then we can write $x = \displaystyle\sum_{i=1}^k \lambda_i x_i$ and $y = \displaystyle\sum_{j=1}^\ell \mu_j y_j$ where $x_i, y_j \in X$, $\lambda_i, \mu_j \geq 0$ for all $i,j$, and $\displaystyle\sum_{i=1}^k \lambda_i = 1, \displaystyle\sum_{j=1}^\ell \mu_j = 1$. Then for any $\alpha \in [0,1]$, we have
\[
\alpha x + (1-\alpha) y = \alpha \sum_{i=1}^k \lambda_i x_i + (1-\alpha) \sum_{j=1}^\ell \mu_j y_j = \sum_{i=1}^k (\alpha \lambda_i) x_i + \sum_{j=1}^\ell ((1-\alpha) \mu_j) y_j.
\]
Since $\alpha \lambda_i\geq 0, (1-\alpha) \mu_j \geq 0$ for all $i,j$ and $\displaystyle\sum_{i=1}^k (\alpha \lambda_i) + \displaystyle\sum_{j=1}^\ell ((1-\alpha) \mu_j) = 1$, we have $\alpha x + (1-\alpha) y \in \conv(X)$, which shows that $\conv(X)$ is convex.
\end{solution}
...
\end{solution} 
  ...
  Then $x = \begin{pmatrix}|&|&\hdots &|\\a_1 & a_2 & \hdots & a_k \\ |&|&\hdots &|\end{pmatrix} \begin{pmatrix}\lambda_1 \\ \lambda_2 \\ \vdots \\ \lambda_k\end{pmatrix}$.
