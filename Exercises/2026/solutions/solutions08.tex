...
\item We show that the number of iterations is at most $2\log_2(a)$. Let $a_0 = a, b_0 = b$ and for each $i \in \mathbb{N}$, let $a_i = b_{i-1}$ and $b_i = r_{i-1}$ where $r_{i-1}$ is the remainder obtained in the $i$-th iteration. We have that $a_i = q_i b_i + r_i$ for some $q_i, r_i \in \mathbb{N}$ such that $b_i > r_i \geq 0$. By part (i), we have $r_i \leq a_i/2$. Hence $a_{i+1} = b_i \leq a_i/2$. We conclude that $a_i \leq a/2^i$. Hence if $i > 2\log_2(a)$, then $a_i < 1$, which is a contradiction since $a_i$ is a natural number. Therefore the number of iterations is at most $2\log_2(a)$, and the number of bit operations is at most $2\log_2(a)$ as well.
\end{enumerate}
\end{solution}
