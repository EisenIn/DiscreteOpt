...
\begin{enumerate}[i)]
	...
	\item Using the strong duality theorem, show that there exists $\lambda \in \R^m_{\geq 0}$ such that $(\lambda \;\, 1)^T \begin{pmatrix} A \\ \alpha^T \end{pmatrix} = 0$ and $(\lambda \;\, 1)^T \begin{pmatrix} b \\ \beta \end{pmatrix} < 0$.
	\item Using i) and ii), conclude the proof of Farkas' lemma by induction on $m$.
\end{enumerate}
