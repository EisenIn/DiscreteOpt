...
\item In this exercise, a number  $a ∈ ℕ$ is represented as a string $s_k,s_{k-1},\dots,s_0$, $s_i ∈ \{0,1\}$. Access to the $i$-th bit $s_i$ takes one unit of time. 
...
\item Recall the \emph{sieve of Erathostenes}. This algorithms starts with listing the natural numbers from $2$ to $k$, 
  \begin{displaymath}
    L = 2,3,4, \cdots, k. 
  \end{displaymath}
  In the beginning, all numbers are colored green.

  The algorithm now iterates the following step until there is no green number left.
  \begin{quote}    
  ...
  \begin{enumerate}[a)] 
  \item  Assuming that arithmetic operations on $n$-bit numbers can be performed in time $O(n)$, show that the sieve of Erathostenes runs in time $O(k^2 \log k)$.
  \item Using the \emph{prime number theorem}, explain that the first $n$ prime numbers can be generated in time
    \begin{displaymath}
      O(n \log^2 n).  
    \end{displaymath}
  \end{enumerate}
...
\item { Testing ${\rank(A) = n}$}
  ...
  \smallskip
  \noindent 
  Let $A ∈ ℤ^{n ×n}$ with $|a_{ij}| ≤ B$ where here, we assume $B ≥n$. In the lecture we have seen that Gaussian elimination operates on rational numbers or bitlength bounded by $O(n \log B)$. Assuming that arithmetic operations can be carried out in linear time, this amounts to a total running time of
  \begin{displaymath}
    O(n ^3 ⋅n  \log B) =  O(n ^4  \log B). 
  \end{displaymath}
We can therefore decide in this running time, whether $A$ is non-singular. 
...
The goal of this exercise is to design a faster randomized algorithm that tests whether $\rank(A) =n$ holds. The running time of the algorithm is
 \begin{displaymath}
  ...
  If $\rank(A) =n$, the algorithm will err (assert $\rank(A) = n$) with probability at most $1/2$.  
  
  \begin{enumerate}
  \item Assuming $B ≥n$, show that $|\det(A)| ≤ B^{2n}$.
  \item If $\det(A) ≠0$, then at least half of the first $2 ⋅ (\log B) ⋅ 2n$ prime numbers do not divide $\det(A)$.
  \item Assuming that choosing a random prime from the $2 ⋅ (\log B) ⋅ 2n$ prime numbers can be done in time $1$, describe a randomized test for $\det(A) =0$ that runs in the desired time. You can also assume that arithmetic operations in $ℤ_p$ can be done in time $\log p$. 
  \end{enumerate}
