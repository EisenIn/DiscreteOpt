...
The goal of this exercise is to design a faster randomized algorithm that tests whether $\rank(A) =n$ holds. The running time of the algorithm is
 \begin{displaymath}
  ...
  \item Assuming that choosing a random prime from the $2 ⋅ (\log B) ⋅ 2n$ prime numbers can be done in time $1$, describe a randomized test for $\det(A) =0$ that runs in the desired time. You can assume that arithmetic operations in $ℤ_p$ can be done in time $\log p$. 
  \end{enumerate}
