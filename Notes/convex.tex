...
We now describe convex sets that are generated by a set 
 $X\subseteq\setR^n$  of 
vectors of $ℝ^n$. The \emph{linear hull}, \emph{affine hull}, \emph{conic hull} and \emph{convex
...
\begin{figure}[htbp]
  \begin{center}
    %\resizebox{4cm}{4cm}{%
    \includegraphics[width=7cm]{Ipe/separation.pdf}
  %}
  \end{center}
  \caption{An illustration of the proof of Theorem~\ref{conv:thr:10}.}
\end{figure}
...
\begin{proof}
  ...
  compact, there exists a point $y^* \in K$ with minimal distance to
  $x^*$. By Exercise~\ref{conv:it:9} we can translate $K$ to $K-y^*$ and $x^*$ to $x^* - y^*$ which justifies that we can assume without loss of generality that $y^*=0$ holds. 


  Consider the hyperplane $a^Tx = \beta$ with $a = x^*$ and $\beta = 1/2 ⋅ \|x^*\|^2$.%  Clearly, $a^T x^* = \beta - 1/2 \|k^*-x^*\|^2$ and $a^T k^*
  % = \beta +  1/2 \|k^*-x^*\|^2$.
  Clearly $a^T x^* > β$. Suppose now that there exists a point $k ∈ K$ with $a^T k ≥ β$. By convexity of $K$,  the line segment spanned by $0$ and $k$ is contained in $K$, i.e.,   $λ k ∈ K $ for each $λ ∈ [0,1]$. The distance squared between $x^*$ and a point $λ ⋅ k$ is 
  \begin{eqnarray*}
    g(λ) & = &  \| x^* - λ k \|^2  \\
         & = &  \| x^*\|^2 - 2λ ⋅ k^T {x^*}  +   λ^2\| k \|^2
  \end{eqnarray*}
The derivative of this function is 
   \begin{eqnarray*}
    g'(λ)         & = &  - 2  k^T x^* +  2 λ \| k \|^2. 
   \end{eqnarray*}
   One has $g'(0) = - 2 k^T {x^*}  ≤ -2 β <0$. 
  As a function of $\lambda$, $g(λ)$ is decreasing 
  at $\lambda=0$. Thus there exists a point on the line segment between $0$ and $k$ which is closer to $x^*$ than $0$ and this is a contradiction. 
  \qed 
\end{proof}
...
\begin{definition}
  \label{def:4}
  A hyperplane $a^T x = β$ as in Theorem~\ref{conv:thr:10} is called a \emph{separating hyperplane} for $x^*$ and $K$. 
\end{definition}
...
\item Let $K ⊆ ℝ^n$ be a convex set and $x^*, y^* ∈ ℝ^n$. The hyperplane $a^Tx = β$ is a separating hyperplane for $x^*$ and $K$ if and only of the hyperplane
  $a^Tx = β - a^T y^*$ is a separating hyperplane for the convex set $K - y^*$ and $x^* - y^*$. \label{conv:it:9} 
