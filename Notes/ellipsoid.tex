...
\begin{displaymath}
   A \cdot U = \mat{A' &|&   0} 
\end{displaymath}
 where $A'$ has full column rank. The
...
\section{Balls, ellipsoids and the half-ball lemma}
...
Recall the following notion from linear algebra. A symmetric matrix $W
\in \setR^{n\times n}$ is called positive definite if all its eigenvalues are
positive.  Recall the following theorem. 
\begin{theorem}
  \label{el:thr:22}
  Let $W \in \setR^{n\times n}$ be a symmetric matrix. The following are
  equivalent. 
  \begin{enumerate}[i)]
  \item $W$ is positive definite. 
  \item $W = L^TL $, where $L\in \setR^{n\times n} $ is a uniquely determined upper
    triangular matrix. 
  \item $x^TWx>0$ for each $x \in \setR^n \setminus\{0\}$. 
  \item $W = Q^T \textrm{diag}(\lambda_1,\ldots,\lambda_n) Q$, where $Q \in \setR^{n\times n}$ is an
...
ellipsoid. An ellipsoid $\eE(W,a)$ is the set  $\eE(W,a) = \{ x \in \setR^n \mid
(x-a)^TW^{-1}(x-a)\leq 1\}$, where $W \in \setR^{n\times n}$ is a symmetric positive
definite matrix and $a\in \setR^n$ is a vector, called the 
\emph{center} of the ellipsoid.  Consider the half-ellipsoid
$\eE(W,a) \cap (c^Tx\leq c^Ta)$.

Our goal is a similar lemma as the half-ball-lemma for ellipsoids.
We know  that each half-ellipsoid $\eE(W,a) \cap
(c^Tx\leq c^Ta)$ must be contained in another ellipsoid $\eE(W',b')$
with $\vol(\eE(W',a'))/ \vol(\eE(W,a))\leq e^{-1/(2n)}$.  More precisely
...
We now record the formula for the ellipsoid $\eE'(W',a')$. 
It is defined by 
\begin{eqnarray}
  \label{el:eq:20}
  a' &  = & a - \frac{1}{n+1}b \\
  W' & = & \frac{n^2}{n^2-1}\left( W - \frac{2}{n+1}b\, b^T   \right),
\end{eqnarray}
where $b$ is the vector $b = W\, c/ \sqrt{c^TW\,c} $.  
...
\begin{lemma}[Half-Ellipsoid-Theorem]
  \label{el:thr:20}
  The half-ellipsoid $\eE(W,b) \cap (c^Tx\leq c^Ta)$ is contained in the
  ellipsoid $\eE'(W',a')$ and one has $\vol(\eE')/
  \vol(\eE)\leq e^{-1/(2n)}$. 
\end{lemma}
...
\section{The ellposloid method for convex sets} 
\label{el:sec:method}

Let $K ⊆ ℝ^n$ be a bounded convex set and suppose that this convex set contained in an initial ellipsoid an ellipsoid $\eE_{init} ⊇ K$. Furthermore, let $L ∈ ℚ_+$ be a positive number. The ellipsoid method that is described below ends with the following finding.

\begin{enumerate}[i)] 
\item The method  either finds a point $x^* ∈ K$ or \label{item:e:1}
\item or asserts that the volume of $K$ satisfies \label{item:e:2}
  \begin{displaymath}
    \vol(K) < L. 
  \end{displaymath}
\end{enumerate}
...
\item If $a \in K$, then return $x^* = a$ and stop  \label{el:item:1} 
\item If $\vol(\eE)<L$, then stop with assertion $\vol(\eE)<L$. \label{el:item:3}
\item Otherwise, compute an inequality $c^Tx\leq\beta$ which is valid for
  $K$  and satisfies $c^Ta>\beta$ and replace $\eE(A,a)$ by $\eE(A',a')$
...
\begin{remark}
  \label{rem:1}
  Notice that the separation theorem  (Theorem~\ref{conv:thr:10}) guarantees the existence of such an inequality  $c^Tx\leq\beta$ in step~\ref{el:item:4} of the algorithm. 
\end{remark}


\begin{theorem}
  \label{el:thr:21}
  The ellipsoid method computes a point $x^* ∈ K$   or
  asserts that $\vol(K) <L$ in  
  \begin{displaymath}
      2\cdot n \ln (\vol(\eE_{init}) / L)
    \end{displaymath}
    iterations. 
\end{theorem}
...
\section{Linear programming: Deciding feasibility}
