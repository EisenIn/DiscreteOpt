...
\begin{proof}
    Suppose there exists $t \in V$ such that 
   ...
   Suppose that one has, for all $v ∈ V$ $f_n(v) = f_{n-1}(v)$ and
   let $C = v_0,v_1,\cdots,v_k,v_0$ be a cycle 
   ...
   \begin{displaymath}
      0 = \sum_{i = 0}^k f_n(v_{i+1}) - f_{n-1}(v_i) \leq
...
\item \label{arbitrage} The \emph{node-edge incidence matrix} $A_G ∈ \{0, \pm 1\}^{V ×A}$ of a directed graph $G = (V,A)$   is defined as
  \begin{displaymath}
    a_{ua} = \begin{cases}
