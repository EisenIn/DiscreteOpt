...
An \emph{optimization problem} is a patr $(ℱ,c)$, where $ℱ$ is a set, referred to as the set of \emph{feasible solutions},  and $c: ℱ ⟶ ℝ$ is a function, that is called the \emph{objective} or \emph{cost function}. The problem is to find an element $x ∈ ℱ$ such that
\begin{displaymath}
  c(x) ≤ c(y) \text{ holds for all } y ∈ ℱ. 
\end{displaymath}
Such an $x∈ ℱ$  is then an \emph{optimal solution} of the optimization problem.
Since the task is to find a minimum solution, we also write 
\begin{displaymath}
  \min \{ c(x) : e ∈ ℱ\}. 
\end{displaymath}
...
\begin{example}
  \label{exe:1}
  From linear algebra we know the problem of minimizing a \emph{quadratic form} $ x^T A x$, where $A ∈ ℝ^{d×d}$ is a symmetric matrix,  over the sphere $S^{(d-1)}  = \{ x ∈ ℝ^d ： \|x\| = 1 \} ⊆ ℝ^d$.  Thus, in this case,  $ℱ = S^{(d-1)}$ and $c(x) = x^T A x$.

If $v ∈ ℝ^d \setminus \{0\}$ is an eigenvector corresponding to the minimal eigenvalue $λ_{\min}$ of $A$, then $v \ \|v\|$ is an optimal solution of the optimization problem. 
