...
\subsection{Full-dimensionality}
\label{el:sec:full-dimensionality}
...
Here we have a third version of Farkas' Lemma (we saw the firt one 
in Theorem~\ref{conv:thr:12} and the second one in 
Exercise~\ref{conv:ex:10} of chapter~\ref{conv:cha:convex-sets} and 
in Theorem~\ref{dual:thr:2farkas}).
\begin{theorem}
  \label{el:thr:24}
...
Let $A\in \setZ^{m\times n}$ and $b\in \setZ^m$ and let $B$ be the largest absolute
value of a coefficient of $A$ and $b$.  If $Ax\leq b$ is not feasible,
then there exists a $\lambda\geq0$ such that $\lambda^T(A|b) = (\mathbf{0}|-1)$. We
want to estimate the largest absolute value of a coefficient of $\lambda$
with Cramer's rule and the Hadamard inequality.  We can choose $\lambda$
such that the  nonzero
coefficients of $\lambda$ are the unique solution of a system of equations
$Cx=d$, where each coefficient has absolute value at most $B$. By
Cramer's rule and the Hadamard inequality we can thus  choose $\lambda$
such that $|\lambda(i)| \leq (n\cdot B)^n$.  Now let $\varepsilon = 1 / \left((n+1) \cdot (n \cdot
  B)^n \right) $. Then $|\lambda^T \mathbf{1} \cdot \varepsilon |<1$ and thus 
\begin{equation}
  \label{el:eq:42}
...
if $Ax\leq b$ is infeasible.  Notice again that the encoding length of
